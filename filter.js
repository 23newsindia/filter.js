...
initializeComponents() {
    ...
    // ONLY ADD DESKTOP CLASSES HERE
    el.classList.add('nasa-top-sidebar-desktop'); // New class for desktop control
    
    // REMOVE THIS LINE COMPLETELY:
    // el.classList.add('nasa-active');
});
    ...
    if (!this.topSidebar.querySelector('.row')) {
        ...
        Array.from(this.topSidebar.children).forEach(child => {
            if (
                ...
                child.id === 'nasa_woocommerce_filter_variations-4' // Specific widget ID
            ) {
    ...
    // Remove hidden-tag from mobile toggle (if needed)
    document.querySelectorAll('.toggle-topbar-shop-mobile.hidden-tag').forEach(el => {
        if (isMobile) {
            el.classList.remove('hidden-tag');
        }
    });
...
// Add the new helper method outside initializeComponents()
updateMobileElementsVisibility() {
    ...
    // 1. Create content box if missing
    ...
    // 3. Add open/close switch and wrap content
    let openToggle = widget.querySelector('.nasa-open-toggle');
    if (!openToggle) {
        openToggle = document.createElement('div');
        openToggle.className = 'nasa-open-toggle'; // Remove 'widget-hidden' to make it open by default
        openToggle.style.display = 'block'; // Ensure it's visible by default
        
        // Move existing content into the toggle wrapper
    ...
    // 4. Ensure title comes after toggle button
    ...
    // 5. Initialize price slider if needed
...
setupPriceFilter(widget) {
    ...
    widgetElements.forEach((widget, index) => {
        ...
        if (!this.widgets.has(type)) {
            // Initialize widget content (toggle button, wrapper, etc.)
    ...
    if (resultCount) {
        ...
        // Add clear filters button for desktop
...
setupEventListeners() {
    ...
    // Single document click handler for all other events
    document.addEventListener('click', (e) => {
        // Filter clicks
        const filterLink = e.target.closest('.nasa-filter-by-attrs, .nasa-filter-by-variations');
        if (filterLink) {
            e.preventDefault();
            e.stopPropagation();
        ...
        if (toggleBtn) {
            ...
            if (openToggle.style.display === 'none') {
                openToggle.style.display = 'block'; // Open the section
                toggleBtn.classList.remove('nasa-hide');
            } else {
                openToggle.style.display = 'none'; // Close the section
...
initializeSlider(range, rangeBar, handleMin, handleMax, values) {
 ...
 setupMobileFilters() {
    ...
    if (window.matchMedia("(max-width: 767px)").matches) {
        this.topSidebar.classList.remove('nasa-active'); // Ensure closed initially
        this.topSidebar.classList.add('nasa-mobile-filter'); // Mobile-specific class
    }
...
// In your initializeComponents() method, after removing hidden-tag:
// At the end of initializeComponents()
